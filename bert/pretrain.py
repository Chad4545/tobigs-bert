...
def seek_random_offset(f, back_margin=2000):
    ...
    f.seek(0, 2)    # 포인터를 제일 마지막으로
    # we remain some amount of text to read
    max_offset = f.tell() - back_margin # f.tell(): 포인터의 제일 마지막 위치 - back_margin 뒤로 마진만큼 이동
    f.seek(randint(0, max_offset), 0)   # 포인터를 처음 0 ~ max_offset 중 랜덤하게 선택
    f.readline() # throw away an incomplete sentence <- 랜덤한 포인터라서 처음 문장이 완성되지 않을 수 있음
...
class Preprocess4Pretrain(Pipeline):
    ...
    def __call__(self, instance):
        ...
        shuffle(cand_pos)
        # 마스킹은 전체 단어의 15% 정도를 진행하는데,
        # 여기에는 재밌게도 모든 토큰을 마스킹 한게 아니라 80% 정도만 <MASK>로 처리하고
        ...
        for pos in cand_pos[:n_pred]:
            ...
            if rand() < 0.8: # 80%
                ...
                tokens[pos] = get_random_word(self.vocab_words) # randomly choose one word
