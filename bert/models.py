...
from bert.utils import split_last, merge_last
...
class MultiHeadedSelfAttention(nn.Module):
    ...
    def __init__(self, cfg):
        super().__init__()
        # 물론 저마다 각각 다른 초기화로 인해 실제로는 서로 다른 값에서 출발하지만 입력값의 구성은 동일하게 시작한다.
        # BERT는 이처럼 동일한 토큰이 문장내의 다른 토큰에 대한 Self-Attention 효과를 갖는다.
        self.proj_q = nn.Linear(cfg.dim, cfg.dim)   # 디코더의 Hidden State <- 디코더가 없음으로 q,k,v의 초기값이 동
        self.proj_k = nn.Linear(cfg.dim, cfg.dim)   # 인코더의 Hidden State
        self.proj_v = nn.Linear(cfg.dim, cfg.dim)   # v는 k에 부여 받운 Normalized Weight, 초기값은 v와 k가 동
